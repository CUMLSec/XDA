...
class RobertaModel(FairseqLanguageModel):
    ...
    def register_classification_list(self, name, num_classes=None, inner_dim=None, **kwargs):
        """Register a sequence of classification heads."""
        if name in self.classification_heads:
            prev_num_classes = self.classification_heads[name].out_proj.out_features
            prev_inner_dim = self.classification_heads[name].dense.out_features
            if num_classes != prev_num_classes or inner_dim != prev_inner_dim:
        ...
        self.classification_heads[name] = RobertaClassificationList(
            self.args.encoder_embed_dim,
            inner_dim or self.args.encoder_embed_dim,
            num_classes,
            self.args.pooler_activation_fn,
...
class RobertaClassificationList(nn.Module):
    ...
    def forward(self, features, **kwargs):
        x = features  # bsz, src_len, input_dim
        x = self.dropout(x)
        x = self.dense(x)
        x = self.activation_fn(x)
        x = self.dropout(x)
