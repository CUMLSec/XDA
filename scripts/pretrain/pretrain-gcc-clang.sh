...
rm checkpoints/pretrain_gcc_clang -rf

mkdir -p checkpoints/pretrain_gcc_clang
...
CUDA_VISIBLE_DEVICES=0,1,2,3 python train.py \
  ...
  --no-epoch-checkpoints --save-dir checkpoints/pretrain_gcc_clang/ \
  --memory-efficient-fp16 \
  --mask-prob 0.2 --random-token-prob 0.5 |
  tee -a result/pretrain_gcc_clang
